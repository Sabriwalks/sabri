...
**tts-2's faster default pace is the intended behavior, not a bug to fix.** The ~16% faster default speech pace measured for `inworld-tts-2` vs. `inworld-tts-1.5-max` (identical text, same voice) was flagged for a listen-check rather than assumed fine. Resolved: Aharon confirmed the faster pace is preferred — it matches his own habit of manually speeding up 1.5-max. No code change; client-side `playbackRate` (already independent of TTS provider/model) is untouched.

**Language selection expanded from 6 fixed languages to Inworld's 15 GA languages, with regional Spanish/Portuguese variants.** The 6-language ceiling was never a model/API limit — confirmed live that Portuguese and Mexican-Spanish voices already worked on both `inworld-tts-1.5-max` and `inworld-tts-2`; it was purely Sabri's own hardcoded config (`LANGUAGE_NAMES`, `INWORLD_LANGUAGE_VOICE_MAP`, 2 hardcoded `<option>` lists). Restricted to Inworld's 15 formally-tested (GA) languages, not the wider experimental tier, per the "test new language pairs with real audio" standing practice. Spanish and Portuguese get real regional variants (Castilian/Latin American, Brazilian/European) — the only 2 of the 15 with more than one real accent option in the catalog and near-term relevance (South America trip); every other language keeps one voice, same as the original 6.

Architecture choices made along the way:
- **`LANGUAGE_CATALOG`** (`server.js`) is the new single source of truth for the dropdown's contents, injected client-side the same way `INWORLD_LANGUAGE_VOICE_MAP` already was — replaces the two hand-maintained `<option>` lists in `index.html` (which had already drifted into a duplication smell) with one server-side array, populated into both selects at runtime by a shared `populateLanguageSelect` function in `app.js`.
- **Nested UI reuses existing primitives, not a new widget.** No comparable nested-choice pattern existed elsewhere in the app to extend, so this uses the same native `<select>` element and `.hidden`-class-toggle convention already used everywhere else in the codebase — a second `<select>` that only appears (populated from the chosen language's `variants`) for Spanish/Portuguese, and stays hidden with zero options for every single-voice language.
- **`LANGUAGE_NAMES` keeps the generic display name for every variant** (`"es"`/`"es-MX"` both read "Spanish"; `"pt-BR"`/`"pt-PT"` both read "Portuguese") rather than a per-variant name — `buildGenderConsistencyGuidance` and other callers do exact string comparisons against these names, so a per-variant rename would have silently broken that matching. Regional steering (Mexican vs. Castilian vocabulary/register, Brazilian vs. European) is instead a separate instruction layered into `buildLanguageGuidance`, keyed off the raw language code — same pattern as the existing Arabic MSA-lock decision.
- **`buildGenderConsistencyGuidance` extended to Portuguese** (same self-reference adjective/participle agreement as French/Spanish) since it's the same well-established grammatical pattern already validated for that branch. Italian (same pattern) and Hindi (closer to the Hebrew/Arabic verb-conjugation pattern) were deliberately left uninvestigated rather than guessed at, per the "empirical testing over assumption" practice — a known gap, not an oversight.
- **Existing `"es"`/`"fr"` voice picks (Marta/Hélène) were kept unchanged**, not re-picked, specifically so already-saved user profiles with bare `"es"` continue to resolve exactly as before.
- **`LANGUAGE_VOICE_MAP`** (the OpenAI-path fallback voice map) was deliberately **not** expanded to the new languages — OpenAI TTS is the non-default rollback path with no verified per-language voice-quality data for the new set, and guessing entries would be exactly the kind of unverified assumption the standing practice warns against. New languages fall back to the user's own Settings voice choice (or the OpenAI default) if `TTS_PROVIDER` is ever flipped back — graceful, not a regression, but not comprehensively tuned either.
- **`SPEECH_RECOGNITION_LANGS`** (`app.js`) — found and fixed as a real gap while tracing `settings.language`'s call sites, not named in the original request: without BCP-47 entries for the new codes, voice Q&A input for any new-language user would have silently tried to recognize speech as English.
- **Onboarding's conversational language extraction** (`ONBOARDING_CHAT_SYSTEM_PROMPT`) needed its own update — it independently sets `settings.language` via a separate Claude-driven conversation extraction, not the Settings/Preferences dropdowns, and had its own hardcoded 6-language instruction.
- **Camera-identify (`/api/identify`) genuinely reuses the shared `buildLanguageGuidance`/`LANGUAGE_NAMES` machinery**, confirmed by re-reading its call site rather than assumed — but its call to `buildLanguageGuidance` was passing only the display name, not the raw language code, meaning it would have silently missed the new regional (Mexican/Castilian/Brazilian/European) steering specifically while still picking up the base language correctly. Fixed as part of this same change.

Real-audio spot-check (light, not exhaustive): sample clips generated for German, Italian, Brazilian Portuguese, Japanese, and Hindi via the real `/api/ask` → `/api/speak` pipeline (Claude-written text, not hand-typed) and sent for Aharon's ear. The remaining 4 of the 9 newly-added languages (Chinese, Dutch, Korean, Polish) are shipping on Inworld's GA claim and the catalog-description voice picks alone — not independently listen-checked.
