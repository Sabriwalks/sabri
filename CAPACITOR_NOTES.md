...
| `SpeechRecognition` / `webkitSpeechRecognition` | **No.** iOS Safari/WKWebView has historically had little-to-no support for the Web Speech *recognition* API (as opposed to speech *synthesis*, which is unrelated and not used here). | **Swapped everywhere.** `@capacitor-community/speech-recognition` installed; `app.js` has a `SabriSpeechRecognition` abstraction (same runtime-detection pattern as geolocation). The core "Talk to Sabri" tour Q&A flow (`startListening()`/`stopListening()`/`cancelListening()`), and the onboarding-chat/planner-chat mic buttons (via the shared `createChatMicController()` helper) all route through it now — no code left talking to the raw Web Speech API `recognition` object directly. The onboarding/planner conversion happened after real-world beta testing surfaced two bugs from the old direct-`recognition`-usage pattern (unreliable pickup, mistimed stop-listening) caused by no silence-timer fallback plus multiple flows silently overwriting each other's event handlers on the same shared object — see the comment above `createChatMicController` in `app.js` for the full story. **See "First things to test in Xcode" below — the native branch itself is still unverified on a real device and remains the single biggest functional risk in the conversion.** |
...
1. **Speech recognition (`SabriSpeechRecognition`, `app.js`).** Tap the mic
   ...
   **Rollback note:** because everything routes through the
   ...
   that consumes a transcript are unaffected either way. This now also
   covers the onboarding-chat and planner-chat mic buttons, which were
   converted to the same abstraction later — worth a quick spot-check on
   each of those too, not just the core tour flow's mic.
