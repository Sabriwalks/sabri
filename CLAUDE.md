...
- **PWA/service-worker update propagation:** update-detection has needed more than one round of fixing, and has silently failed in the field even after passing sandbox testing — don't trust a fix here without real-device evidence. Current mechanism: a periodic `registration.update()` check while the tab stays visible, not just on visibility-transition (`checkForUpdateAndMaybeApply` / `UPDATE_CHECK_INTERVAL_MS` in app.js). The magenta boot-screen diagnostic used to test this was reverted in `333a984` (cache bumped to `sabri-cache-v37`) — it is no longer live, don't look for it. Result as of 2026-09-07 (Aharon's real-device recollection, not a controlled test): the "Update available" banner fired correctly once during an actual real update and hasn't reappeared since, because no update has shipped since to trigger it — this confirms the periodic check does detect and surface a real update in the field. The auto-apply/self-reload path specifically (vs. just detection) was never independently confirmed via the diagnostic and has had no other real-world trigger since — treat that half as still unconfirmed if it becomes relevant again.
